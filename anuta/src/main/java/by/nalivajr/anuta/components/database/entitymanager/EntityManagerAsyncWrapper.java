...
/**
 * Provides a simple implementation of {@link AnutaAsyncEntityManager}, wrapping an existing instance of
 * {@link AnutaEntityManager}. Supports both sync and async modes
 */
public class EntityManagerAsyncWrapper implements AnutaAsyncEntityManager {

    public static final int POOL_SIZE = 5;
    private final AnutaEntityManager entityManager;
    private ExecutorService executorService;

    public EntityManagerAsyncWrapper(AnutaEntityManager entityManager) {
    ...
    @Override
    public <T> void findByQuery(final AnutaQuery<T> query, ActionCallback<List<T>> callback) {
        executorService.submit(createTask(new Callable<List<T>>() {
            @Override
            public List<T> call() throws Exception {
    ...
    @Override
    public <T> void getEntityCursor(final AnutaQuery<T> query, ActionCallback<AnutaEntityCursor<T>> callback) {
        executorService.submit(createTask(new Callable<AnutaEntityCursor<T>>() {
            @Override
            public AnutaEntityCursor<T> call() throws Exception {
    ...
    @Override
    public <T> void executeQuery(final AnutaQuery<T> query, final ActionCallback<Boolean> callback) {
        executorService.submit(createTask(new Callable<Boolean>() {
            @Override
            public Boolean call() throws Exception {
                return entityManager.executeQuery(query);
